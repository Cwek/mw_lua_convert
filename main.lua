...
--]]
    ...
    function Convert.out(args)
        ...
        local processCount=args["processCount"]
        ...
        if disp=="output only" then
            ...
            for k,v in pairs(in_num) do
                table.insert(out,in_num[k])
                if processCount>1 and embellish[k]~=nil then--1.processCount>1则是没embellish，2.embellish[k]为nil则无该项（embellish只有args["processCount"]-1个元素）
                    table.insert(out,embellish[k])
                end
                if inunit_code[k]~=nil then
            ...
            for k,v in pairs(out_num) do
                table.insert(out,out_num[k])
                if processCount>1 then
                    if embellish[k]~=nil then
                        table.insert(out,embellish[k])                    
                    else
                        table.insert(out,"，")
                    end                   
                end
                if outunit_code[k]~=nil then
                    table.insert(out,inunit_code[k])
                end
            end            
